...
func NewHandler(
	...
	registerRoute(mux, "GET /api/wildcard", api.WildcardStatus, readOnly, "", authManager, browserStore)
	registerRoute(mux, "POST /api/wildcard/{action}", api.WildcardToggle, readWrite, "wildcard_toggle", authManager, browserStore)
	registerRoute(mux, "PUT /api/wildcard/domains", api.WildcardDomainsReplace, readWrite, "wildcard_domains_replace", authManager, browserStore)
